...
app_dis_get_data_cb app_dis_user_callback = NULL;
                                          ...
                                          ke_task_id_t const src_id)
{
    ...
    if(app_dis_user_callback == NULL)
    {
        ...
        app_dis_get_data send_data = {0};
        app_dis_user_callback(param->value, &send_data);
        data = send_data.data_p;
        len  = send_data.data_len;
    }
...
void app_dis_add_dis(void)
{
    ...
    #if (BLE_APP_AM0)
    req->sec_lvl =  (uint8_t)(SVC_SEC_LVL(NOT_ENC)| ATT_UUID(128));//PERM(SVC_AUTH, NO_AUTH);
    #else
...
void app_dis_reg_callback(app_dis_get_data_cb cb)
{
    app_dis_user_callback = cb;
}
