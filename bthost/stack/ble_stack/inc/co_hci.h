...
#ifndef BLE_STACK_PORTING_CHANGES
...
enum hci_opcode
{
    ...
    HCI_LE_SET_DATA_RELATED_ADDR_CHANGE_CMD_OPCODE      = 0x207C,
    HCI_LE_SET_DEFAULT_SUBRATE_CMD_OPCODE               = 0x207D,
    HCI_LE_SUBRATE_REQUEST_CMD_OPCODE                   = 0x207E,
    #if (mHDT_LE_SUPPORT)
    TCI_DBG_MHDT_LE_RD_RM_FEAT_CMD_OPCODE               = 0x20FF,
    #endif
...
#if (mHDT_LE_SUPPORT)
    TCI_LE_SET_DEFAULT_LABLE_PARAMS_CMD_OPCODE     = 0xFC34,
#endif
    ...
    #if (mHDT_LE_SUPPORT)
    TCI_DBG_MHDT_LE_CMD_OPCODE                     = 0xFC5B,
    #endif
    ...
    #if (mHDT_LE_SUPPORT)
    TCI_DBG_MHDT_LE_RD_LOC_FEAT_CMD_OPCODE        = 0xFCE1,
    #endif
};
...
enum hci_le_evt_subcode
{
    ...
    HCI_LE_SUBRATE_CHANGE_EVT_SUBCODE          = 0x23,
};

#if (mHDT_LE_SUPPORT)
/*@TRACE*/
enum tci_mhdt_le_evt_subcode
{
    TCI_MHDT_LE_RD_LOC_PROPRIETARY_FEAT_CMP_EVT_SUBCODE     = 0x05,
    TCI_MHDT_LE_RD_RM_PROPRIETARY_FEAT_CMP_EVT_SUBCODE      = 0x04,
};
#endif
...
/// HCI basic command complete event structure with connection handle
/*@TRACE*/
struct hci_basic_conhdl_le_cmd_cmp_evt
{
    ///LE Subevent code
    uint8_t     subcode;
    /// status
...
/// HCI command structure for the LE set default subrate command
/*@TRACE*/
struct hci_le_set_default_subrate_cmd
{
    /// subrate min
...
/// HCI command structure for the LE subrate request command
/*@TRACE*/
struct hci_le_subrate_request_cmd
{
    /// Connection handle
    uint16_t        conhdl;
    /// subrate min
...
/// HCI command strucutre for the LE subrate change Reporting event
/*@TRACE*/
struct hci_le_subrate_change_rep_evt
{
    /// LE Subevent code
    uint8_t         subcode;
    /// Status
...
#if (mHDT_LE_SUPPORT)
///TCI_LE_Set_Default_Label_Parameters command
/*@TRACE*/
// 5B
struct tci_le_set_default_label_params_cmd
{
    /// allow ISO_Interval/FT and CIG_Event window to change.
    /// (true/false)
    uint8_t keep_sdu_sync_ref_constant;
...
///TCI_LE_Read_Local_Proprietary_Feature command
/*@TRACE*/
// B
struct tci_le_rd_local_proprietary_feat_cmd
{
    uint8_t subcode;
};

///TCI_LE_Read_Remote_Proprietary_Feature command
/*@TRACE*/
// BH
struct tci_le_rd_remote_proprietary_feat_cmd
{
    uint16_t conhdl;
    uint8_t subcode;
};

///TCI_LE_Read_Remote_Proprietary_Feature_CMD complete Event
/*@TRACE*/
// BBHL
struct tci_le_rd_rm_proprietary_feat_cmp_evt
{
    uint8_t status;
    uint16_t conhdl;
    uint32_t features;
...
///TCI_LE_Read_Local_Proprietary_Feature_CMD complete Event
/*@TRACE*/
// BBL
struct tci_le_rd_local_proprietary_feat_cmp_evt
{
    uint8_t status;
    uint32_t features;
};
...
struct hci_le_set_per_adv_en_cmd
{
     ...
     *  Bit number 1: Include ADI field in AUX_SYNC_IND PDUs (BT_53)
...
/// This command is used to read the TX_Time_Stamp and Time_Offset of a transmitted SDU identified by the Packet_Sequence_Number on a CIS or BIS
/// identified by the Connection_Handle parameter on the Central or Peripheral.
/*@TRACE*/
struct hci_le_rd_iso_tx_sync_cmd
{
    /// Connection handle of the CIS or BIS. (Range 0x0000-0x0EFF)
    uint16_t conhdl;
};
...
/// Command Complete event of HCI_LE_Read_ISO_TX_Sync
/*@TRACE*/
struct hci_le_rd_iso_link_tx_sync_cmd_cmp_evt
{
    /// 0x00 - Command succeeded ; 0x01-0xFF Failed reason
    uint8_t  status;
    /// Connection handle of the CIS or BIS (12 bits meaningful - Range 0x0000-0x0EFF)
    ...
    /// The time offset, in microseconds, that is associated with a transmitted SDU.(0x000000 to 0xFFFFFF)
    uint32_t time_offset;
};
...
typedef void (*iap_dbg_ble_cis_established_info_record_cb)(uint8_t con_lid, struct hci_le_cis_established_evt const* cis_info);
void iap_dbg_ble_cis_established_evt_cb_register(iap_dbg_ble_cis_established_info_record_cb func);
#endif
/// @} CO_BT
#endif // CO_HCI_H_
