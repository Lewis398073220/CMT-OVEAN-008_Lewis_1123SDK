...
/// Structure for OTC_GET_CFG command message
typedef struct otc_get_cfg_cmd
{
    /// Command code (see enum #otc_cmd_codes)
    uint16_t cmd_code;
    /// Connection local index
...
typedef struct otc_cmp_evt
{
    ...
    {
        /// Indication-capable characteristic type (see enum #otp_char_type)
        /// For OTC_GET_CFG and OTC_SET_CFG command codes
        uint8_t char_type;
        /// Filter local index
...
/// Structure for OTC_CFG indication message
typedef struct otc_cfg_ind
{
    /// Indication code (see enum #otc_msg_ind_codes)
    uint16_t ind_code;
    /// Connection local index
    ...
    /// Indication-capable characteristic type (see enum #otp_char_type)
    uint8_t char_type;
    /// Indicate if sending of indications is enabled (!= 0) for the indicated characteristic
    uint8_t enabled;
} otc_cfg_ind_t;
