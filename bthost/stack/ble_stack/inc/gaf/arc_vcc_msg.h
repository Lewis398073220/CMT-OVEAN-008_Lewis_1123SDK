...
enum arc_vcc_msg_cmd_code
{
    ...
    /// Get Configuration (see #arc_vcc_get_cfg_cmd_t)
    ARC_VCC_GET_CFG = GAF_CODE(ARC, VCC, ARC_VCC_CMD_TYPE_GET_CFG),
...
enum arc_vcc_msg_ind_code
{
    ...
    /// Configure (see #arc_vcc_cfg_ind_t)
...
{
    /// Command code (shall be set to #ARC_VCC_GET_CFG)
    uint16_t cmd_code;
    /// Connection local index
    uint8_t con_lid;
    /// Characteristic type
...
{
    /// Indication code (shall be set to #ARC_VCC_CFG)
    uint16_t ind_code;
    /// Connection local index
    uint8_t con_lid;
    /// Characteristic type
