...
typedef struct bap_bc_deleg_restore_bond_data_req
{
    ...
    uint16_t cli_cfg_bf;
    /// Event bit field (1 bit per Broadcast Receive State characteristic instance) indicating characteristics whose
    /// value has changed between disconnection and reconnection and for which sending of a notification is needed
    uint16_t evt_bf;
