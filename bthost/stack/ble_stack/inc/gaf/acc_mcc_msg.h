...
enum acc_mcc_msg_cmd_code
{
    ...
    /// Get Configuration (see #acc_mcc_get_cfg_cmd_t)
    ACC_MCC_GET_CFG = GAF_CODE(ACC, MCC, ACC_MCC_CMD_TYPE_GET_CFG),
...
enum acc_mcc_msg_ind_code
{
    ...
    /// Configuration (see #acc_mcc_cfg_ind_t)
...
{
    /// Command code (shall be set to #ACC_MCC_GET_CFG)
    uint16_t cmd_code;
    /// Connection local index
    uint8_t con_lid;
    /// Media local index
...
{
    /// Indication code (shall be set to #ACC_MCC_CFG)
    uint16_t ind_code;
    /// Connection local index
    uint8_t con_lid;
    /// Media local index
