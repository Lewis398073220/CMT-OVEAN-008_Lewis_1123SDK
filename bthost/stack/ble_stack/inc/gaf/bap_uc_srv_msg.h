...
enum bap_uc_srv_msg_cmd_code
{
    ...
    /// Get ISO TX Sync
    BAP_UC_SRV_GET_ISO_TX_SYNC = GAF_CODE(BAP, UC_SRV, BAP_UC_SRV_CMD_TYPE_GET_ISO_TX_SYNC),
};
...
enum bap_uc_srv_msg_ind_code
{
    ...
    /// CIS rejected (see #bap_uc_srv_cis_rejected_ind_t)
    BAP_UC_SRV_CIS_REJECTED = GAF_CODE(BAP, UC_SRV, 3),
    /// CIG terminated (see #bap_uc_srv_cig_terminated_ind_t)
    BAP_UC_SRV_CIG_TERMINATED = GAF_CODE(BAP, UC_SRV, 4),
#ifdef BLE_STACK_PORTING_CHANGES
    /// ASE ntf value (see #bap_uc_srv_ase_ntf_value_ind_t)
    BAP_UC_SRV_ASE_NTF_VALUE = GAF_CODE(BAP, UC_SRV, 5),
#endif
};
...
} __attribute__ ((__packed__)) bap_uc_srv_get_quality_cmp_evt_t;

typedef struct bap_uc_srv_get_iso_tx_sync_cmd
{ 
    uint16_t cmd_code;
    /// ASE local index
    uint8_t ase_lid;
} bap_uc_srv_get_iso_tx_sync_cmd_t;

typedef struct bap_uc_srv_get_iso_tx_sync_cmp_evt
{
    uint16_t cmd_code;
    /// Status
    uint16_t status;
...
} bap_uc_srv_get_iso_tx_sync_cmp_evt_t;
...
typedef struct bap_uc_srv_cis_state_ind
{
    ...
    iap_ug_config_t cig_config;
    /// Stream configuration\n
    /// Meaningful only if conhdl is not GAP_INVALID_CONHDL
    iap_us_config_t cis_config;
    /// status
    uint8_t status;
    /// reason
    uint8_t reason;
...
/// Structure for #BAP_UC_SRV_CIS_REJECTED indication message
typedef struct bap_uc_srv_cis_rejected_ind
{
    ...
    /// Reject reason
    uint8_t error;
} bap_uc_srv_cis_rejected_ind_t;

/// Structure for #BAP_UC_SRV_CIG_TERMINATED indication message
typedef struct bap_uc_srv_cig_terminated_ind
{
    ...
    uint8_t reason;
} bap_uc_srv_cig_terminated_ind_t;

#ifdef BLE_STACK_PORTING_CHANGES
/// Structure for #BAP_UC_SRV_ASE_NTF_VALUE indication message
typedef struct bap_uc_srv_ase_ntf_value_ind
{
    ...
    /// Reason
    uint8_t reason;
} bap_uc_srv_ase_ntf_value_ind_t;
#endif
