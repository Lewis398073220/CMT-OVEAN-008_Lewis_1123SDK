...
enum bap_uc_cli_msg_cmd_code
{
    ...
    /// Get Configuration (see #bap_uc_cli_get_cfg_cmd_t)
...
enum bap_uc_cli_msg_ind_code
{
    ...
    /// Configuration (see #bap_uc_cli_cfg_ind_t)
    ...
    /// Server disconnected
    /// ACL Disconnected
    BAP_UC_CLI_UNBOND_ASE = GAF_CODE(BAP, UC_CLI, 9),
};
...
/// Structure for #BAP_UC_CLI_GET_CFG command message (see \ref bap_msc_ascs_get_set_cfg)
typedef struct bap_uc_cli_get_cfg_cmd
{
    /// Command code (shall be set to #BAP_UC_CLI_GET_CFG)
    uint16_t cmd_code;
    /// Connection local index
    uint8_t con_lid;
    /// Characteristic type
...
/// Structure for #BAP_UC_CLI_SET_CFG command message (see \ref bap_msc_ascs_get_set_cfg)
...
{
    /// Indication code (set to #BAP_UC_CLI_CFG)
    uint16_t ind_code;
    /// Connection local index
    uint8_t con_lid;
    /// Characteristic type
