...
enum bap_bc_assist_msg_ind_code
{
    ...
    /// Indicate notification configuration for an instance of the Broadcast Receive state characteristic
    /// (see #bap_bc_assist_cfg_ind_t)
...
{
    /// Command code (shall be set to #BAP_BC_ASSIST_GET_CFG)
    uint16_t cmd_code;
    /// Connection local index
    uint8_t con_lid;
    /// Source local index
...
{
    /// Indication code (set to #BAP_BC_ASSIST_CFG)
    uint16_t ind_code;
    /// Connection local index
    uint8_t con_lid;
    /// Source local index
