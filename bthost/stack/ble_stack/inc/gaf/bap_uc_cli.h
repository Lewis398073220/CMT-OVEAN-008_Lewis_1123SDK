...
enum bap_uc_cli_cmd_type
{
    ...
    /// Get Configuration
    BAP_UC_CLI_CMD_TYPE_GET_CFG = BAP_UC_CLI_CMD_TYPE_GET_QUALITY + 2,
    /// Set Configuration
...
typedef void (*bap_uc_cli_cb_state_empty)(uint8_t con_lid, uint8_t ase_instance_idx, uint8_t ase_lid, uint8_t direction, uint8_t state);
#else
...
/**
 ****************************************************************************************
 * @brief Callback function called when acl disconnected
 *
 * @param[in] con_lid           Connection local index
 *                              GAF_INVALID_LID indicates that the ASE is no more used
 * @param[in] ase_instance_idx  ASE instance index
 ...
 ****************************************************************************************
 */
typedef void (*bap_uc_cli_cb_unbond_ase)(uint8_t con_lid, uint8_t ase_instance_idx, uint8_t ase_lid);
...
typedef struct bap_uc_cli_cb
{
    ...
    /// Callback function called when acl disconnected
    bap_uc_cli_cb_unbond_ase  cb_unbond_ase;
} bap_uc_cli_cb_t;
...
uint16_t bap_uc_cli_restore_bond_data_codec(uint8_t con_lid, uint8_t ase_instance_idx, uint8_t ase_lid,
                                            uint8_t dp_id, gaf_codec_id_t* p_codec_id,
