...
enum arc_micc_msg_cmd_code
{
    ...
    /// Get Configuration (see #arc_micc_get_cfg_cmd_t)
    ARC_MICC_GET_CFG = GAF_CODE(ARC, MICC, ARC_MICC_CMD_TYPE_GET_CFG),
...
enum arc_micc_msg_ind_code
{
    ...
    /// Configuration (see #arc_micc_cfg_ind_t)
...
{
    /// Command code (shall be set to #ARC_MICC_GET_CFG)
    uint16_t cmd_code;
    /// Connection local index
    uint8_t con_lid;
...
{
    /// Indication code (shall be set to #ARC_MICC_CFG)
    uint16_t ind_code;
    /// Connection local index
    uint8_t con_lid;
    /// Enabled or disabled
