...
enum tmap_tmac_msg_cmd_code
{
    ...
    /// Telephony and Media Audio Service Client - Get Role (see #tmap_tmac_get_role_cmd_t)
    TMAP_TMAC_GET_ROLE = GAF_CODE(TMAP, TMAC, TMAP_TMAC_CMD_TYPE_GET_ROLE),
};
...
/// Structure for #TMAP_TMAC_GET_ROLE command message
typedef struct tmap_tmac_get_role_cmd
{
    /// Command code (shall be set to #TMAP_TMAC_GET_ROLE)
    uint16_t cmd_code;
    /// Connection local index
    uint8_t con_lid;
} tmap_tmac_get_role_cmd_t;
