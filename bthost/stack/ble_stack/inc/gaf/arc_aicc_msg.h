...
enum arc_aicc_msg_cmd_code
{
    ...
    /// Get Configuration (see #arc_aicc_get_cfg_cmd_t)
    ARC_AICC_GET_CFG = GAF_CODE(ARC, AICC, ARC_AICC_CMD_TYPE_GET_CFG),
...
enum arc_aicc_msg_ind_code
{
    ...
    /// Configuration (see #arc_aicc_cfg_ind_t)
...
{
    /// Command code (shall be set to #ARC_AICC_GET_CFG)
    ///  - ARC_AICC_GET_CFG
    uint16_t cmd_code;
    /// Connection local index
    uint8_t con_lid;
...
{
    /// Indication code (shall be set to #ARC_AICC_CFG)
    ///  - ARC_AICC_CFG
    uint16_t ind_code;
    /// Connection local index
    uint8_t con_lid;
