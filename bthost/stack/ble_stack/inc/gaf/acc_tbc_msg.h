...
enum acc_tbc_msg_cmd_code
{
    ...
    /// Get Configuration (see #acc_tbc_get_cfg_cmd_t)
    ACC_TBC_GET_CFG = GAF_CODE(ACC, TBC, ACC_TBC_CMD_TYPE_GET_CFG),
...
enum acc_tbc_msg_ind_code
{
    ...
    /// Configuration (see #acc_tbc_cfg_ind_t)
...
{
    /// Command code (shall be set to #ACC_TBC_GET_CFG)
    uint16_t cmd_code;
    /// Connection local index
    uint8_t con_lid;
    /// Bearer local index
...
{
    /// Indication code (shall be set to #ACC_TBC_CFG)
    uint16_t ind_code;
    /// Connection local index
    uint8_t con_lid;
    /// Bearer local index
