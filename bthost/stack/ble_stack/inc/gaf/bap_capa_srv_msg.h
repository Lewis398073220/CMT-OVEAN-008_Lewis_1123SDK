...
enum bap_capa_srv_msg_ind_code
{
    ...
    /// Location (see #bap_capa_srv_location_ind_t message structure)
    BAP_CAPA_SRV_LOCATION = GAF_CODE(BAP, CAPA_SRV, 1),
    /// CCCD Written (see #bap_capa_srv_cccd_written_ind_t message structure)
    BAP_CAPA_SRV_CCCD_WRITTEN = GAF_CODE(BAP, CAPA_SRV, 2),
};
...
/// Structure for #BAP_CAPA_SRV_CCCD_WRITTEN indication message
typedef struct bap_capa_srv_cccd_written_ind
{
    /// Indication code (shall be set to #BAP_CAPA_SRV_CCCD_WRITTEN)
    uint16_t ind_code;
    /// Connection local index
    uint8_t con_lid;
} bap_capa_srv_cccd_written_ind_t;
