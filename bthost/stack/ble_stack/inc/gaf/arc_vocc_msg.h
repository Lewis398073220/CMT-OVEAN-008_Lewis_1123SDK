...
enum arc_vocc_msg_cmd_code
{
    ...
    /// Get Configuration (see #arc_vocc_get_cfg_cmd_t)
    ARC_VOCC_GET_CFG = GAF_CODE(ARC, VOCC, ARC_VOCC_CMD_TYPE_GET_CFG),
...
enum arc_vocc_msg_ind_code
{
    ...
    /// Configuration (see #arc_vocc_cfg_ind_t)
...
{
    /// Command code (shall be set to #ARC_VOCC_GET_CFG)
    uint16_t cmd_code;
    /// Connection local index
    uint8_t con_lid;
    /// Output local index
...
{
    /// Indication code (shall be set to #ARC_VOCC_CFG)
    uint16_t ind_code;
    /// Connection local index
    uint8_t con_lid;
    /// Output local index
