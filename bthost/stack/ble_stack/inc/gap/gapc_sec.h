 ...
 * \snippet{lineno} app_test_bt_periph.c APP_BT_SEC_CB
 * \endif
 *
 * @{
 * @}
...
typedef struct gapc_security_cb
{
     ...
     * Request shall be accept using #gapc_encrypt_accept or reject using #gapc_encrypt_reject
     *
     ...
     ****************************************************************************************
     */
    void (*encrypt_req)(uint8_t conidx, uint32_t dummy, uint16_t ediv, const rand_nb_t* p_rand);
     ...
     * @param[in] conidx         Connection index
     * @param[in] dummy          Dummy parameter provided by upper layer application
     * @param[in] level          Pairing level achieved (@ref gap_pairing_lvl)
     * @param[in] is_ltk_present Is LTK exchanged during pairing.
     *
     ****************************************************************************************
     */
     ...
     * @param[in] oob_present  OOB data presence (see enum #gap_bt_oob_flag)
     ****************************************************************************************
     */
    void (*peer_iocap)(uint8_t conidx, uint32_t dummy, uint8_t iocap, uint8_t auth_req_bf, uint8_t oob_present);
     ...
     *
     * @note Required callback of legacy pairing supported.
     *
     * @param[in] conidx        Connection index
     * @param[in] dummy         Dummy parameter provided by upper layer application
     ...
     *
     * @note Mandatory callback if LE pairing supported
     *
     * @param[in] conidx        Connection index
     * @param[in] dummy         Dummy parameter provided by upper layer application
 ...
 ****************************************************************************************
 */
uint16_t gapc_encrypt(uint8_t conidx, uint32_t dummy, const gapc_ltk_t* p_ltk_info, gapc_proc_cmp_cb cmp_cb);

/**
 ****************************************************************************************
 * @brief On LE connection, this function shall be used to accept encryption request from peer device
 *
 * @note Once link is encrypted, #gapc_security_cb_t.auth_info is called
 *
 * @param[in] conidx     Connection index
 ...
 ****************************************************************************************
 */
uint16_t gapc_encrypt_accept(uint8_t conidx, const gap_sec_key_t* p_ltk, uint8_t key_size);

/**
 ****************************************************************************************
 * @brief On LE connection, this function shall be used to reject encryption request from peer device
 *
 * @param[in] conidx     Connection index
 *
 * @return Execution status (see enum #hl_err).
 ****************************************************************************************
 */
uint16_t gapc_encrypt_reject(uint8_t conidx);
 ...
 *
 * @note PIN Code pointer information must stay valid and available after function execution (global variable)
 *
 * @param[in] conidx        Connection index
