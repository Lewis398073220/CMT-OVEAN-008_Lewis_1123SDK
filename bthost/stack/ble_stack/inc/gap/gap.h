...
#define GAP_BLE_MIN_OCTETS  (27) // number of octets
/// Minimum data packet transmission duration
#define GAP_BLE_MIN_TIME    (328) // in us
/// Maximum data packet transmission size
#define GAP_BLE_MAX_OCTETS  (251) // number of octets
/// Maximum data packet transmission duration
#define GAP_BLE_MAX_TIME    (17040) // in us
...
enum gap_role
{
    ...
    /// Device supports all LE roles: both peripheral and central
    GAP_ROLE_ALL         = (GAP_ROLE_CENTRAL | GAP_ROLE_PERIPHERAL),
    /// Device supports BT Classic only
    GAP_ROLE_BT_CLASSIC  = 0x10,
    /// Device supports full BTDM configuration
    GAP_ROLE_BTDM        = (GAP_ROLE_ALL | GAP_ROLE_BT_CLASSIC),
};
...
enum gap_adv_filter_policy
{
    ...
    GAP_ADV_ALLOW_SCAN_ANY_CON_ANY    = 0x00,
...
{
    /// Men in the middle rrotection required
    GAP_BT_AUTH_MITM_REQ_POS       = 0,
    GAP_BT_AUTH_MITM_REQ_BIT       = 0x01,
    /// Dedicated bonding (cannot be set locally, forced to zero)
    GAP_BT_AUTH_DEDICATED_BOND_POS = 2,
    GAP_BT_AUTH_DEDICATED_BOND_BIT = 0x04,
    /// General Bonding
...
enum gap_auth
{
    ...
    /// SEC_CON and Bonding and no MITM
    GAP_AUTH_REQ_SEC_CON_BOND_NO_MITM = (GAP_AUTH_SEC_CON | GAP_AUTH_BOND),

    GAP_AUTH_REQ_LAST,

    /// Mask of  authentication features without reserved flag
#ifdef BLE_STACK_PORTING_CHANGES
    GAP_AUTH_REQ_MASK             = 0x3F,
#else
...
/*@TRACE*/
typedef struct gap_periph_pref
{
...
/*@TRACE*/
typedef struct gap_le_con_param
{
...
typedef struct gap_le_con_param_nego
{
...
/*@TRACE*/
typedef struct gap_ral_dev_info
{
    /// Device identity
    gap_bdaddr_t addr;
