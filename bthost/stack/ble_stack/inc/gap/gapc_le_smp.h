...
#include "gap_int.h"
#include "l2cap_ble.h"
#include "l2cap_smp.h" // Should Only be included there (L2CAP protocol defines)
#include "co_buf.h"
 ...
 ****************************************************************************************
 */
typedef uint16_t (*gapc_le_smp_proc_transition_handler)(gapc_le_smp_pair_proc_t* p_proc, uint8_t event, bool* p_is_finished);

/// Pairing procedure information
typedef struct gapc_le_smp_pair_proc
{
    /// Procedure transition callback
    gapc_le_smp_proc_transition_handler transition_cb;
    /// transaction timer
...
gapc_le_smp_pair_proc_t* gapc_get_pairing_proc(uint8_t conidx);

/// Check if authentication flag is valid
__STATIC_FORCEINLINE bool gapc_le_smp_is_auth_valid(uint8_t auth)
{
...
void gapc_le_smp_start_trans_timer(gapc_le_smp_pair_proc_t* p_proc);

/// @brief Stop the SMP Transaction timer timer
void gapc_le_smp_stop_trans_timer(gapc_le_smp_pair_proc_t* p_proc);
