...
/// additional information bit field
enum gapc_add_info_bf
{
    /// Peer device initiates LE connection update
    GAPC_PEER_LE_CONN_UP_POS          = 0,
    GAPC_PEER_LE_CONN_UP_BIT          = 0x01,
    /// Local device initiates LE connection update
    GAPC_LOCAL_LE_CONN_UP_POS         = 1,
    GAPC_LOCAL_LE_CONN_UP_BIT         = 0x02,
};
...
#ifdef BLE_STACK_PORTING_CHANGES
    /// Used to record the CT2 bit
    GAPC_AUTH_REQ_CT2_POS      = 6,
    GAPC_AUTH_REQ_CT2_BIT      = 0x0040,
#endif
};
...
#ifdef BLE_STACK_PORTING_CHANGES
/// GATT service configuration bit field
///
///     7    6  5     4        3       2     1      0
/// +-------+--+--+------+----------+-----+------+-----+
/// | RFU                                        | DB H|
/// +-------+--+--+------+----------+-----+------+-----+
///
/// - Bit [0]  : DB hash characteristic is diabled
/// - Bit [1-7]  : RFU
enum gatt_svc_cfg_bf
{
    /// DB hash Characteristic present
    GATT_SVC_DB_HASH_CHAR_PRESENT_BIT      = 0x01,
    GATT_SVC_DB_HASH_CHAR_PRESENT_POS      = 0,
};
#endif
...
typedef struct gapc_con
{
    ...
    uint8_t                      info_bf;
    /// Additional information fields (see enum #gapc_add_info_bf)
    uint8_t                      add_info_bf;
} gapc_con_t;
...
typedef struct gapc_env_
{
    ...
    #endif // (BLE_PWR_CTRL)
    #if (BLE_SUBRATE)
    /// LE subrate change event callbacks
    gapc_le_subreta_change_cb        p_le_sub_change_cb;
    #endif // (BLE_SUBRATE)
    ...
    uint8_t                          cfg_flags;
#ifdef BLE_STACK_PORTING_CHANGES
    /// GATT service cfg bf @see gatt_svc_cfg_bf
    uint8_t                          gatt_svc_cfg_flags;
#endif
} gapc_env_t;
 ...
 ****************************************************************************************
 */
__STATIC_FORCEINLINE gapc_con_t* gapc_con_env_get(uint8_t conidx)
{
    return ((conidx < HOST_CONNECTION_MAX) ? gapc_env.p_con[conidx] : NULL);
}

#ifdef BLE_STACK_PORTING_CHANGES
bool gapc_is_ct2_support(uint8_t conidx);
#endif
 ...
 ****************************************************************************************
 */
const gap_bdaddr_t* gapc_get_bdaddr(uint8_t conidx, uint8_t src);
