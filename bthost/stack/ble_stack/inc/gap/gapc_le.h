...
/// @addtogroup GAPC_LE_SUBRATE_API LE Subrate
/// @{

/**
 ****************************************************************************************
 * @brief Set default_subrate detection
 *
 * @param[in] dummy           Upper layer software dummy parameter returned each time an event occurs on connection
 * @param[in] sub_min         Subrate min
 * @param[in] sub_max         Subrate max
 * @param[in] lat_max         Latency max
 * @param[in] cont_num        Continuation Number
 * @param[in] timeout         Supervision timeout (timeout * 10ms)
 * @param[in] cmp_cb          Function called when procedure is over.
 *
 * @return Return function execution status (see enum #hl_err)
 *         If returns GAP_ERR_NO_ERROR, upper layer SW shall wait for #gapc_proc_cmp_cb callback execution
 ****************************************************************************************
 */
uint16_t gapc_le_set_default_subrate(uint32_t dummy, uint16_t sub_min, uint16_t sub_max,
                                        uint16_t lat_max, uint16_t cont_num, uint16_t timeout, gapc_proc_cmp_cb cmp_cb);
 ...
 * @param[in] dummy       Dummy parameter provided by upper layer application
 * @param[in] status      Status of procedure execution (see enum #hl_err)
 * @param[in] sub_factor  Subrate Factor
 * @param[in] per_latency Peripheral_Latency
 * @param[in] cont_num    Continuation Number
 * @param[in] timeout     Supervision timeout (timeout * 10ms)
 *
 ****************************************************************************************
 */
typedef void (*gapc_le_subreta_change_cb)(uint8_t conidx, uint32_t dummy, gapc_subrate_change_report_t *p_para);

/**
****************************************************************************************
* @brief Set Callback that will handle subrate change reports
*
* @param[in] p_cbs         Pointer to the callback set
*
* @return Return function execution status (see enum #hl_err)
****************************************************************************************
*/
uint16_t gapc_subrate_change_set_callbacks(const gapc_le_subreta_change_cb p_cb);

/**
 ****************************************************************************************
 * @brief Subrate request detection
 *
 * @param[in] conidx          Connection index
 * @param[in] dummy           Upper layer software dummy parameter returned each time an event occurs on connection
 * @param[in] sub_min         Subrate min
 ...
 ****************************************************************************************
 */
uint16_t gapc_le_subrate_request(uint8_t conidx, uint32_t dummy, uint16_t sub_min, uint16_t sub_max,
                                        uint16_t lat_max, uint16_t cont_num, uint16_t timeout, gapc_le_subreta_change_cb cmp_cb);
