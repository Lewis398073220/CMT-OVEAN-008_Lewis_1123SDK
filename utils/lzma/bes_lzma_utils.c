...
#if (RAM_SIZE < 0x80000)
#error "OTA_BIN_COMPRESS need more memory for malloc(may need set CP_IN_SAME_EE=0)"
#endif
...
void ota_copy_compressed_image(uint32_t srcFlashOffset, uint32_t dstFlashOffset)
{
    ...
    if (srcbuf)
    {
        ...
        memcpy((void*)srcbuf,(const uint8_t*)SRC_FLASH_LOGIC_ADDR + srcFlashOffset + readOffset,otaBootInfo.imageSize-readOffset);
...
#endif
    {
        lock = int_lock();
        pmu_flash_write_config();
        memcpy(decompressed,(const uint8_t*)DST_FLASH_LOGIC_ADDR + dstFlashOffset,FLASH_SECTOR_SIZE_IN_BYTES);
