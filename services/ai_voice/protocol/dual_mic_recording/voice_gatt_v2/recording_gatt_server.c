                              ...
                              uint8_t user_prio, const void *params, const void *p_cb)
{
    ...
    PRF_ENV_T(ai) *sv_env = (PRF_ENV_T(ai) *)bes_ble_gap_ke_malloc(sizeof(PRF_ENV_T(ai)), KE_MEM_PROFILE);
    memset((uint8_t *)sv_env, 0, sizeof(PRF_ENV_T(ai)));

    do
    {
        /// registor GATT server user
        status = bes_ble_gatt_user_srv_register(PREFERRED_BLE_MTU, user_prio,
        ...
        status = bes_ble_gatt_db_svc_add(sv_env->srv_user_lid, sec_lvl,
 ...
 ****************************************************************************************
 */
static void recording_create(prf_data_t* p_env, uint8_t conidx, const gap_le_con_param_t* p_con_param)
{
                             ...
                             uint16_t hdl, uint16_t offset, uint16_t max_length)
{
    ...
    if (VOICE_IDX_CMD_TX_NTF_CFG == hdl - voice_env->shdl)
    {
        ...
        bes_ble_gap_prf_buf_alloc(&p_data, dataLen);
    ...
    bes_ble_gatt_srv_att_read_get_cfm(conidx, user_lid, token, status, dataLen, p_data);

    // Release the buffer
    bes_ble_gap_prf_buf_release(p_data);
}
...
static void _cb_att_info_get(uint8_t conidx, uint8_t user_lid, uint16_t token, uint16_t hdl)
{
    ...
    bes_ble_gatt_srv_att_info_get_cfm(conidx, user_lid, token, status, length);
}
                            ...
                            uint16_t hdl, uint16_t offset, co_buf_t *p_buf)
{
    ...
    bes_ble_gatt_srv_att_val_set_cfm(conidx, user_lid, token, status);
}
...
static void send_notifiction(uint8_t conidx, uint8_t contentType, const uint8_t *ptrData, uint32_t length)
{
    ...
    {
        co_buf_t *p_buf = NULL;
        bes_ble_gap_prf_buf_alloc(&p_buf, length);
        ...
        bes_ble_gatt_srv_event_send(conidx, voice_env->srv_user_lid, dummy, GATT_NOTIFY,
                            voice_env->shdl + handle_offset, p_buf);

        /// Release the buffer
        bes_ble_gap_prf_buf_release(p_buf);
    }
}
                            ...
                            uint32_t length)
{
    ...
    if (0xFFFF != handle_offset)
    {
        ...
        bes_ble_gatt_srv_event_send(conidx, voice_env->srv_user_lid, dummy, GATT_INDICATE,
                            voice_env->shdl + handle_offset, p_buf);
